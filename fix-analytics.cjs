...
const code = `import React, { useState, useRef, useMemo, useEffect } from 'react';
...
export function AnalyticsScreen({ userId }: AnalyticsScreenProps) {
  ...
  const filteredAccounts = useMemo(() => {
    ...
    switch (activeFilter.type) {
      case 'PAID': return currentMonthAccounts.filter(a => a.status === 'PAID');
      case 'OVERDUE': return currentMonthAccounts.filter(a => a.status === 'PENDING' && isBefore(getJSDate(a.dueDate), now));
      case 'UPCOMING': return currentMonthAccounts.filter(a => {
         if (a.status !== 'PENDING') return false;
         const d = getJSDate(a.dueDate);
         return d >= now && d <= next5Days;
      });
  ...
  }, [currentMonthAccounts, activeFilter, now, next5Days]);
  ...
  // Donut (Status)
  const filteredContasPagas = filteredAccounts.filter(acc => acc.status === 'PAID').reduce((acc, curr) => acc + curr.amount, 0);
  const filteredVencidas = filteredAccounts.filter(acc => acc.status === 'PENDING' && isBefore(getJSDate(acc.dueDate), now)).reduce((acc, curr) => acc + curr.amount, 0);
  const filteredAVencer = filteredAccounts.filter(acc => acc.status === 'PENDING' && !isBefore(getJSDate(acc.dueDate), now)).reduce((acc, curr) => acc + curr.amount, 0);

  const donutData = [
    { name: 'Pagas', value: filteredContasPagas, color: '#10b981', type: 'PAID' },
    { name: 'Vencidas', value: filteredVencidas, color: '#ef4444', type: 'OVERDUE' },
    { name: 'A Vencer', value: filteredAVencer, color: '#f59e0b', type: 'UPCOMING' },
  ].filter(d => d.value > 0);
  if (donutData.length === 0) donutData.push({ name: 'Vazio', value: 1, color: '#e2e8f0', type: 'ALL' });

  const handleDonutClick = (data: any, index: number) => {
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-10" ref={chartRef}>
      ...
      <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          {activeFilter && (
            <button onClick={() => setActiveFilter(null)} className="text-xs font-bold text-slate-500 hover:text-slate-800 transition-colors flex items-center gap-1 bg-slate-100 px-3 py-2 rounded-lg">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div 
           className={twMerge("bg-white border rounded-xl p-4 shadow-sm flex flex-col justify-between cursor-pointer transition-all hover:-translate-y-1 hover:shadow-md", isFilterActive('ALL') ? 'border-blue-500 ring-1 ring-blue-500' : 'border-slate-200')}
        ...
        <div 
           className={twMerge("bg-white border rounded-xl p-4 shadow-sm flex flex-col justify-between cursor-pointer transition-all hover:-translate-y-1 hover:shadow-md", isFilterActive('PAID') ? 'border-emerald-500 ring-1 ring-emerald-500' : 'border-slate-200')}
        ...
        <div 
           className={twMerge("bg-white border rounded-xl p-4 shadow-sm flex flex-col justify-between cursor-pointer transition-all hover:-translate-y-1 hover:shadow-md", isFilterActive('OVERDUE') ? 'border-red-500 ring-1 ring-red-500' : 'border-slate-200')}
        ...
        <div 
           className={twMerge("bg-white border rounded-xl p-4 shadow-sm flex flex-col justify-between cursor-pointer transition-all hover:-translate-y-1 hover:shadow-md", isFilterActive('UPCOMING') ? 'border-amber-500 ring-1 ring-amber-500' : 'border-slate-200')}
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col">
          ...
          <div className="flex items-center flex-1">
            ...
            <div className="w-1/2 pl-4 space-y-2">
               <div className={twMerge("flex flex-col p-1.5 -ml-1.5 rounded-lg cursor-pointer transition-colors hover:bg-slate-50", isFilterActive('PAID') ? 'bg-slate-100 ring-1 ring-slate-200' : '')} onClick={() => toggleFilter('PAID')}>
                 ...
                 <span className="text-sm font-medium text-slate-900 ml-4.5">{formatCurrency(filteredContasPagas)}</span>
               </div>
               <div className={twMerge("flex flex-col p-1.5 -ml-1.5 rounded-lg cursor-pointer transition-colors hover:bg-slate-50", isFilterActive('OVERDUE') ? 'bg-slate-100 ring-1 ring-slate-200' : '')} onClick={() => toggleFilter('OVERDUE')}>
                 ...
                 <span className="text-sm font-medium text-slate-900 ml-4.5">{formatCurrency(filteredVencidas)}</span>
               </div>
               <div className={twMerge("flex flex-col p-1.5 -ml-1.5 rounded-lg cursor-pointer transition-colors hover:bg-slate-50", isFilterActive('UPCOMING') ? 'bg-slate-100 ring-1 ring-slate-200' : '')} onClick={() => toggleFilter('UPCOMING')}>
                 ...
                 <span className="text-sm font-medium text-slate-900 ml-4.5">{formatCurrency(filteredAVencer)}</span>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="h-48 w-full cursor-pointer">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={evolutionData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }} onClick={handleBarClick}>
              ...
              />
              <Bar dataKey="Despesas" fill="url(#colorBar)" radius={[6, 6, 0, 0]} maxBarSize={40} className="hover:opacity-80" />
        ...
        <div className="space-y-6">
          {categoryData.length === 0 ? (
            <p className="text-sm text-slate-500 text-center py-4">Nenhum gasto registrado neste período.</p>
             ...
             const catTransactions = (activeFilter?.type === 'CATEGORY' ? currentMonthAccounts : filteredAccounts).filter(a => a.categoryId === cat.id).sort((a,b) => b.amount - a.amount);
...
`;
fs.writeFileSync('src/components/AnalyticsScreen.tsx', code);
