...
export function AnalyticsScreen({ userId }: AnalyticsScreenProps) {
  ...
  const filteredAccounts = useMemo(() => {
    ...
    switch (activeFilter.type) {
      ...
      case 'UPCOMING': return currentMonthAccounts.filter(a => {
         ...
         // "a vencer" is anything pending that is NOT overdue, not just next 5 days.
         return !isBefore(getJSDate(a.dueDate), now);
      });
  ...
  }, [currentMonthAccounts, activeFilter, now]);
            ...
            <div className="w-1/2 h-32 relative flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                ...
                >
                  ...
                  <span className="text-[10px] font-bold text-slate-400 uppercase">{centerTextLabel}</span>
                </div>
              </div>
            </div>
