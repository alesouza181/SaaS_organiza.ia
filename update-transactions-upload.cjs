...
if (!content.includes('import { ref, uploadBytes, getDownloadURL }')) {
  content = content.replace("import { db } from '../firebaseConfig';", "import { db, storage } from '../firebaseConfig';\nimport { ref, uploadBytes, getDownloadURL } from 'firebase/storage';");
}
...
content = content.replace("const [applyToAll, setApplyToAll] = useState(false);", "const [applyToAll, setApplyToAll] = useState(false);\n" + fileUploadHelper);


const formInputOld = `                <div>
...
content = content.replace(formInputOld, formInputNew);
